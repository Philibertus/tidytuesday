...
unique(colonies$year)
unique(colonies$state)

#how do colonies transition over time?
colonies_n_time <- colonies %>%
  group_by(year, state) %>%
  summarize(mean_colony_n = mean(colony_n),
            sd_colony_n = sd(colony_n))

#animation
plot <- colonies_n_time %>%
  dplyr::filter(year != 2019) %>%
  group_by(state) %>%
  dplyr::arrange(desc(mean_colony_n)) %>%
  ggplot(aes(x = year, y = mean_colony_n, group = state, color = factor(state))) +
...
shared_honeybees <- SharedData$new(colonies_n_time)
bscols(widths = c(3,NA,NA),
       list(
         filter_checkbox("state", "State", shared_honeybees, ~state, inline = TRUE),
         filter_slider("mean_colony_n", "Average Colonies #", shared_honeybees, ~mean_colony_n, width = "100%"),
         filter_select("year", "Year", shared_honeybees, ~year)),
       d3scatter(shared_honeybees, ~year, ~mean_colony_n, ~factor(state), width = "100%", height = 250)
       )
